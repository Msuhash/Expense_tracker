...
const ICategoryPieChart = () => {
  const { categoryDistribution, loading, error } = useSelector((state) => state.analytics);

  // Color palette for income categories (greens)
  const COLORS = ['#22c55e', '#16a34a', '#15803d', '#14532d', '#4ade80', '#86efac', '#bbf7d0', '#dcfce7'];

  // Process data for the chart
  const chartData = useMemo(() => {
    if (!categoryDistribution || !categoryDistribution.incomeCategories) return [];

    const data = categoryDistribution.incomeCategories.map((item, index) => ({
      name: item.category,
      value: item.amount,
    ...
    return (
      <div className="flex items-center justify-center w-full min-h-[400px] bg-gray-950 rounded-lg border border-amber-600">
        <div className="text-center">
          ...
          <p className="text-amber-500 italic">Loading income data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <div className="flex items-center justify-center w-full min-h-[400px] bg-gray-950 rounded-lg border border-amber-600">
        <div className="text-center">
          <p className="text-amber-500 italic">No income data available</p>
          <p className="text-amber-700 text-sm mt-2">Add some income to see the distribution</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-950 rounded-lg p-6 border border-amber-800 w-full">
      <div className="mb-4">
        <h3 className="text-2xl font-bold text-green-500">Income Distribution</h3>
      ...
      <div className="mt-4 text-center">
        <p className="text-amber-600 font-semibold">
          Total Income: <span className="text-green-500">₹{total.toLocaleString()}</span>
        </p>
      </div>
    </div>
  );
...
export default ICategoryPieChart;